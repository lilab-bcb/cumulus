...
task run_generate_count_matrix_ADTs {
    ...
    command {
        ...
        call_args = ['python', '/software/convert_to_pdf.py', report_file, '~{sample_id}'+'.report.pdf']
        print(' '.join(call_args))
        check_call(call_args)
...
task run_generate_ridge_plot {
    ...
    command {
        ...
        call_args = ["python", "/software/merge_pdf.py", files_to_merge, "~{sample_id}"+".report_ridgeplot.pdf"]
        print(' '.join(call_args))
        check_call(call_args)
